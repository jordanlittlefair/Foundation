	...
	public:
		...
		void OnUpdate( const Fnd::CommonResources::FrameData& frame_data )
		{
			...
			if ( rb_comp && sc_comp )
			{
				...
				auto world_angular_impulse = Fnd::Math::Vector3( -GetScriptManager()->GetInputHandler()->GetGamePad()->GetAxisValue(Xbox::RightStickY),
																GetScriptManager()->GetInputHandler()->GetGamePad()->GetAxisValue(Xbox::RightStickX),
																-(GetScriptManager()->GetInputHandler()->GetGamePad()->GetAxisValue(Xbox::RightTrigger) - GetScriptManager()->GetInputHandler()->GetGamePad()->GetAxisValue(Xbox::LeftTrigger)))
